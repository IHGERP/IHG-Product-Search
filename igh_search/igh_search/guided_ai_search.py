...
def _promote_query_into_filters(intent):
    ...
    if best_match and best_match["score"] >= 0.8 and (best_match["score"] - second_score >= 0.15 or best_match["score"] == 1.0):
        ...
        if query_variants & value_variants or best_match["score"] >= 1.0:
            intent["query"] = ""
        intent["filters"] = filters
...
def _normalize_voltage_label(value):
    ...
    if not numbers:
        return raw
    suffix = "VAC" if "AC" in raw else "VDC" if "DC" in raw else "V"
    if len(numbers) >= 2 and any(token in raw for token in ["-", "TO"]):
...
def _normalize_current_label(value):
    ...
    if not numbers:
        return raw
    suffix = "A" if raw.endswith("A") and not raw.endswith("MA") else "MA"
    if len(numbers) >= 2 and any(token in raw for token in ["-", "TO"]):
        return f"{_format_number(numbers[0])}{suffix}-{_format_number(numbers[1])}{suffix}"
    return f"{_format_number(numbers[0])}{suffix}"
...
def _build_normalized_suggestions(question_key, facet_counts=None, fallback_values=None, limit=QUESTION_CANDIDATE_LIMIT):
    ...
    for entry in facet_counts or []:
        ...
        if not raw_value:
            continue
        label = _normalize_option_label(question_key, raw_value)
        ...
        if not current:
            groups[token] = {"label": label, "value": raw_value, "count": count, "variants": 1}
            continue
        current["count"] += count
        current["variants"] += 1
    ...
    if not groups:
        for value in fallback_values or []:
            ...
            if token and token not in groups:
                groups[token] = {"label": label, "value": raw_value, "count": 0, "variants": 1}
...
def _score_array_filter(question_key, snapshot, known_values):
...
def _pick_next_question(intent_filters, snapshot, skipped_question=None):
    ...
    for question_key in QUESTION_ORDER:
        ...
        if meta["type"] == "array":
            score, suggestions = _score_array_filter(question_key, snapshot, known_values)
...
def _resolve_array_answer(question_key, answer):
    normalized_answer = cstr(answer or "").strip()
    if not normalized_answer:
        return []

    known_values = get_known_filter_values().get(question_key) or []
    answer_token = _normalize_value_token(question_key, normalized_answer)

    exact_matches = [value for value in known_values if _normalize_value_token(question_key, value) == answer_token]
    if exact_matches:
...
def _apply_answer_to_filters(intent_filters, question_key, answer):
    ...
    if question_key in ARRAY_FILTER_KEYS:
        next_filters[question_key] = _resolve_array_answer(question_key, normalized_answer)
        return next_filters
...
def continue_guided_ai_search(
    ...
    if question_key and not skipped_question and answer_text:
        merged_filters = _apply_answer_to_filters(merged_filters, question_key, answer_text)
